#!/bin/bash
# Unified script for running MTP acceptance rate tests with top-k analysis.
# Calls test_mtp_acceptance_rate_topk.py which reports per-position
# top-1 / top-2 / top-3 acceptance rates.
#
# Usage:
#   bash scripts/run_mtp_benchmark_topk.sh --dataset aime25 --temp 0.6
#   bash scripts/run_mtp_benchmark_topk.sh --dataset gpqa --temp 0.0 --num-spec-tokens 3
#   bash scripts/run_mtp_benchmark_topk.sh --dataset aime25 --temp 1.0 --model-dir /path/to/model
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --help|-h)
            ...
            echo ""
            echo "Runs MTP acceptance rate benchmark with top-k analysis."
            echo "Reports per-position top-1/top-2/top-3 acceptance rates."
            echo ""
...
FNAME="output-topk-${THINK_TAG}-${MAX_TOKENS}-spec${NUM_SPEC_TOKENS}-temp${TEMP}"
